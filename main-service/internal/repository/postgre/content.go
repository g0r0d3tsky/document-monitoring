...
func (s *StorageContent) GetContent(ctx context.Context, id uuid.UUID) (*domain.Content, error) {
...
func (s *StorageContent) DeleteContent(ctx context.Context, filename string) error {
	query := `DELETE FROM "files" WHERE id = $1`

	if _, err := s.db.Exec(ctx, query, filename); err != nil {
